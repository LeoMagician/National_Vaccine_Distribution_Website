...
<body class="page-header-fixed">
                  ...
                  <div class="portlet-body">
                     ...
                     <div class="dataTables_wrapper form-inline" role="grid">
	                     ...
	                     <c:import url = "../shared/paging.jsp">
	        				...
	        				<c:param name="urlAddress" value="/production/list"/>
	       				 </c:import>
       				 </div>
                  </div>
               </div>
   ...
   <script type="text/javascript">
         ...
         $(".table-toolbar").toolbarLite({
             items: [
                 { link: true, display: "Add", css: "icon-plus", showIcon: true, url: "<%=UrlHelper.resolveWithReturnUrl("/production/create", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), pageContext)%>" },
                 { splitter: true }, 
                 { link: true, display: "Edit", css: "icon-edit", showIcon: true, url: "<%=UrlHelper.resolveWithReturnUrl("/production/edit/{0}", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), pageContext)%>", 
                   	selector: "#data-table .checkboxes", mustSelect: "select first!", singleSelect: "Only support single select!"},
                 { splitter: true },                                 
                 { link: true, display: "Delete", css: "icon-trash", showIcon: true, url: "<%=UrlHelper.resolveWithReturnUrl("/production/delete/{0}", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), pageContext)%>", 
                   	selector: "#data-table .checkboxes", mustSelect: "select first!", confirm: "Sure to delete selected data?"}
             ]
         });
      });
   </script>
