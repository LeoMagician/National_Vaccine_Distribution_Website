...
import com.leo.web.models.extension.OrderModelExtension;
...
public class ShoppingCartController extends BaseController{
	...
	public String list(HttpServletRequest request, Model model){
		...
		if(!model.containsAttribute("orderModel")){
			...
			model.addAttribute("orderModel", orderEditModel);
		}
		float priceTotal = 0;
        int pageNo = ServletRequestUtils.getIntParameter(request, PageListUtil.PAGE_NO_NAME, PageListUtil.DEFAULT_PAGE_NO);
        int pageSize = ServletRequestUtils.getIntParameter(request, PageListUtil.PAGE_SIZE_NAME, PageListUtil.DEFAULT_PAGE_SIZE);      
        model.addAttribute("contentModel", orderItemService.listPage(pageNo, pageSize));
        int size = orderItemService.listPage(pageNo, pageSize).getItems().size();
        List<OrderItem> orderitems = orderItemService.listPage(pageNo, pageSize).getItems();
        for(int i=0; i<size; i++){
        	priceTotal += orderitems.get(i).getPrice()*orderitems.get(i).getAmount();
        }
	...
	public String list(HttpServletRequest request, Model model, @ModelAttribute("orderModel") OrderEditModel orderEditModel) throws EntityOperateException, ValidatException{		
		
		String[] oiID = request.getParameterValues("Id"); 
		
		Cart cart = OrderModelExtension.toCart(orderEditModel);
		...
		for(int i=0; i<oiID.length; i++){
			OrderItem oi = orderItemService.get(Integer.valueOf(oiID[i]));
			Vaccine vaccine = vaccineService.get(Integer.parseInt(oi.getVaccineId()));
			vaccine.setAvailability(vaccine.getAvailability()-oi.getAmount());
			oi.setCart(cart);		
