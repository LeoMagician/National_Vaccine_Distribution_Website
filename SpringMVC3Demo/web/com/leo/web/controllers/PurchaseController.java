...
import com.leo.web.models.extension.CartModelExtension;
...
public class PurchaseController extends BaseController{		
	...
	@AuthPassport
	@RequestMapping(value="/add/{vaccineid}", method=RequestMethod.GET)
	public String add(HttpServletRequest request, Model model, @PathVariable(value="vaccineid") Integer id) throws ValidatException{
				
		if(!model.containsAttribute("contentModel"))
		{
			CartEditModel cartEditModel=CartModelExtension.toCartEdit(vaccineService.get(id));
			model.addAttribute("contentModel", cartEditModel);
		}				
        return "purchase/add";
	}
	
	@AuthPassport
	@RequestMapping(value="/add/{vaccineid}", method=RequestMethod.POST)
	public String add(HttpServletRequest request, Model model, @Valid @ModelAttribute("contentModel") CartEditModel editModel, @PathVariable(value="vaccineid") Integer id, BindingResult result) throws EntityOperateException, ValidatException{
				
		if(result.hasErrors()){
            return add(request, model,id);
        }
		
		OrderItem orderItem = CartModelExtension.toOrderItem(editModel);
		ShoppingCart sc = CartModelExtension.toShoppingCart(editModel);
		orderItemService.save(orderItem);
		shoppingCartService.save(sc);
		orderItemService.updateBind(orderItem.getId(), editModel.getVaccineId());		
		shoppingCartService.updateBind(sc.getId(), editModel.getVaccineId());
				
		String returnUrl = ServletRequestUtils.getStringParameter(request, "returnUrl", null);        
        if(returnUrl==null)
        	returnUrl="purchase/list";
    	return "redirect:"+returnUrl;        
	}
