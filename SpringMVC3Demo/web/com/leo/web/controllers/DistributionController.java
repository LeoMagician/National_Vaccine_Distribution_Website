...
import com.leo.web.models.DisOrderModel;
import com.leo.web.models.HospitalEditModel;
import com.leo.web.models.ManuOrderModel;
...
public class DistributionController extends BaseController{
	...
	public String list(HttpServletRequest request, Model model) throws NumberFormatException, ValidatException, EntityOperateException{
		model.addAttribute("requestUrl", request.getServletPath());
		model.addAttribute("requestQuery", request.getQueryString());
				
		List<ManuOrderModel> cvmList = new ArrayList<ManuOrderModel>();
		List<Cart> carts = cartService.listCarts();
		int stateId = 0;
		for(int i=0; i<carts.size();i++){
			ManuOrderModel cvm = new ManuOrderModel();
			cvm.setId(carts.get(i).getId());
			System.out.println("1");
			cvm.setDate(carts.get(i).getDate());
			cvm.setPriceTotal(carts.get(i).getPriceTotal());
			cvm.setOrderStatus(carts.get(i).getStatus());
			Account account = accountService.get(Integer.parseInt(carts.get(i).getAccountId()));
			System.out.println("2");
			cvm.setAccountName(account.getName());
			Hospital hospital = hospitalService.get(Integer.parseInt(account.getHospitalId()));			
			System.out.println("3");
			cvm.setHosName(hospital.getName());									
		...
		if(!model.containsAttribute("contentModel"))
		{
			model.addAttribute("contentModel", cvmList);
		}
	...
	public String add(HttpServletRequest request, Model model, @PathVariable(value="id") Integer id) throws ValidatException, EntityOperateException{
		...
		Account account = accountService.get(Integer.parseInt(cart.getAccountId()));
		Hospital hospital = hospitalService.get(Integer.parseInt(account.getHospitalId()));
		int stateId = hospital.getState().getId();
		if(!model.containsAttribute("contentModel"))
		{
			DisOrderModel editModel = new DisOrderModel();
			model.addAttribute("contentModel", editModel);			
		}
		model.addAttribute(selectDataSourceName, dcService.getSelectStateSource(stateId));
	...
	public String add(HttpServletRequest request, Model model, @Valid @ModelAttribute("contentModel") DisOrderModel editModel, @PathVariable(value="id") Integer id, BindingResult result) throws EntityOperateException, ValidatException {
		if(result.hasErrors())
            return add(request, model, id);
		
		DC dc = dcService.get(editModel.getDcId());
		Cart cart = cartService.get(id);
		cart.setDisEnable(true);
		cart.setDc(dc);
		cart.setStatus("Distributor Approved!");
		cartService.update(cart);
		
		String returnUrl = ServletRequestUtils.getStringParameter(request, "returnUrl", null);        
        if(returnUrl==null)
        	returnUrl="shipping/list";
    	return "redirect:"+returnUrl;
	}
	...
	public String reject(HttpServletRequest request, Model model, @PathVariable(value="id") Integer id) throws ValidatException, EntityOperateException{
		String returnUrl = ServletRequestUtils.getStringParameter(request, "returnUrl", null);
		Cart cart = cartService.get(id);
		cart.setDisEnable(false);
		cart.setStatus("Sorry, Your Vaccine were lost!");
		cart.setDc(null);
		cartService.update(cart);
