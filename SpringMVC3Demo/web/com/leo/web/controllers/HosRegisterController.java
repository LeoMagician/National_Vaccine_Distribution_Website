...
import com.leo.web.models.extension.HospitalModelExtension;

@Controller
@RequestMapping(value="/registration")
public class HosRegisterController extends BaseController{
	...
	public String list(HttpServletRequest request, Model model) throws NoSuchAlgorithmException{
        ...
        model.addAttribute("contentModel", hospitalService.listPage(pageNo, pageSize));
...
//        int userid = accountAuth.getId();
		if(!model.containsAttribute("contentModel"))
		{
			HospitalEditModel hospitalEditModel=new HospitalEditModel();
//			hospitalEditModel.setAccountId(userid);
			model.addAttribute("contentModel", hospitalEditModel);
		}		
//        model.addAttribute("idsk", userid);
    ...
    public String request(HttpServletRequest request, Model model, @Valid @ModelAttribute("contentModel") HospitalEditModel editModel, BindingResult result) throws EntityOperateException, ValidatException {
        if(result.hasErrors()){
...
//        String id = request.getParameter("accId");
        AccountAuth accountAuth = AuthHelper.getSessionAccountAuth(request);        
//        int userid = accountAuth.getId();
        Account account = accountService.get(accountAuth.getId());
        Hospital hospital=HospitalModelExtension.toHospital(editModel);
        hospitalService.save(hospital);        
        account.setHospital(hospital);
        hospitalService.updateBind(hospital.getId(), editModel.getStateId());
        accountService.update(account);
//        hospitalService.hospitalBind(hospital.getId(), 25);  
//        model.addAttribute("idsk", "5");
        String returnUrl = ServletRequestUtils.getStringParameter(request, "returnUrl", null);	
        if(returnUrl==null)
        	returnUrl="registration/list";
    	return "redirect:"+returnUrl;     	
