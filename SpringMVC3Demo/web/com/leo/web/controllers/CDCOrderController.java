...
import com.leo.web.models.CDCOrderModel;


@Controller
@RequestMapping(value="/cdcOrder")
public class CDCOrderController extends BaseController{
	...
	public String list(HttpServletRequest request, Model model) throws NumberFormatException, ValidatException, EntityOperateException{
		...
		List<CDCOrderModel> cvmList = new ArrayList<CDCOrderModel>();
		List<Cart> carts = cartService.listCarts();
		for(int i=0; i<carts.size();i++){
			CDCOrderModel cvm = new CDCOrderModel();
			...
			State state = stateService.get(hospital.getState().getId());
			System.out.println("4");
			List<OrderItem> ois = orderItemService.listOrderItem(carts.get(i).getId());
			System.out.println("5");
			for(int j =0; j<ois.size(); j++){
				OrderItem oi = ois.get(j);
				Quota q = quotaService.listQuotas(state.getId(), oi.getVaccineId());
				System.out.println("6");
				if(oi.getAmount() >= q.getQuota()){
					int exceed = oi.getAmount()-q.getQuota();
					cvm.setQuotaStatus(false);
					cvm.setVaccineAva("Exceed: "+exceed);
					break;
				}else{
					int left = q.getQuota()-oi.getAmount();
					cvm.setQuotaStatus(true);
					cvm.setVaccineAva(left+" vaccine left if the order is approved.");
				}
			}
	...
	public String add(HttpServletRequest request, Model model, @PathVariable(value="id") Integer id) throws ValidatException, EntityOperateException{
		...
		cart.setCdcEnable(true);
		cart.setStatus("CDC Approved!");
		cartService.update(cart);
		
		if(returnUrl==null)
			returnUrl="cdcOrder/list";
		return "redirect:"+returnUrl;
	...
	public String reject(HttpServletRequest request, Model model, @PathVariable(value="id") Integer id) throws ValidatException, EntityOperateException{
		String returnUrl = ServletRequestUtils.getStringParameter(request, "returnUrl", null);
		Cart cart = cartService.get(id);
		cart.setCdcEnable(false);
		cart.setStatus("CDC rejected!");
		cartService.update(cart);
