...
import com.leo.web.models.extension.RecieveModelExtension;


@Controller
@RequestMapping(value="/recieve")
public class HosRecieveController extends BaseController{
	...
	public String list(HttpServletRequest request, Model model) throws NumberFormatException, ValidatException, EntityOperateException{
		...
		List<Cart> carts = cartService.listRecieveCarts();
	...
	public String add(HttpServletRequest request, Model model, @PathVariable(value="id") Integer id) throws ValidatException, EntityOperateException{
		...
		cart.setHosEnable(true);
		cart.setStatus("Vaccine Recieved!");
		cartService.update(cart);				
		int hosinventory = 0;
		System.out.println("1");
		List<OrderItem> ois = orderItemService.listOrderItem(id);
		System.out.println("2");
		for(int i=0; i<ois.size(); i++){
			OrderItem oi = ois.get(i);
			Vaccine vaccine = vaccineService.get(oi.getVaccine().getId());
			System.out.println("1");
			hosinventory = vaccine.getHosInventory()+oi.getAmount();
			System.out.println("1"+hosinventory);
			vaccine.setHosInventory(hosinventory);
			vaccineService.update(vaccine);
		}
				
		if(returnUrl==null)
			returnUrl="recieve/list";
		return "redirect:"+returnUrl;			
	}
}
