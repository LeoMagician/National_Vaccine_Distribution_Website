...
<body class="page-header-fixed">
		                     ...
		                     <table class="table table-striped table-bordered table-hover" id="data-table">
		                        ...
		                        <tbody>
							        ...
							        <tr class="even gradeX">
									    ...
									    <td>${ item.quotaStatus }</td>
									    <td>${ item.vaccineAva }</td>								    				   							            
							        </tr>
							        </c:forEach>
		                        </tbody>
		                     </table>
   ...
   <script type="text/javascript">
         ...
         $(".table-toolbar").toolbarLite({
             items: [                 
                 { link: true, display: "Approve", css: "icon-ok", showIcon: true, url: "<%=UrlHelper.resolveWithReturnUrl("/cdcOrder/approve/{0}", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), pageContext)%>", 
                   	selector: "#data-table .checkboxes", mustSelect: "select first!", singleSelect: "Only support single select!"},
                 { splitter: true },                                 
                 { link: true, display: "Reject", css: "icon-trash", showIcon: true, url: "<%=UrlHelper.resolveWithReturnUrl("/cdcOrder/reject/{0}", request.getAttribute("requestUrl"), request.getAttribute("requestQuery"), pageContext)%>", 
